package com.zimi.zimixing.activity.gpsMonitor.old.amapUtils;
...
public class TraceRePlay implements Runnable {
                ...
                switch (message.what) {
                    case TRACE_MOVE:
                        ...
                        if (trace.mTraceUpdateListener != null) {
                            trace.mTraceUpdateListener.onTraceUpdating(message.arg1, latLng);
                        }
                        break;
                    case TRACE_FINISH:
                        if (trace.mTraceUpdateListener != null) {
    ...
    public void run() {
        synchronized (this) {
            if (mTraceList != null) {
                for (int i = mI; i < mTraceList.size(); i++) {
                    if (mStop) {
                        break;
                    }
